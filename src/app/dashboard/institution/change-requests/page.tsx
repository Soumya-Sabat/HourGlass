...
export default function ChangeRequestsPage() {
  ...
  if (loading) return <div className="border-2 border-[var(--border-primary)] bg-[var(--bg-secondary)] p-6 font-mono shadow-[4px_4px_0px_0px_var(--border-primary)]">Loading change requests...</div>;
    ...
    <div className="space-y-6 font-mono text-[var(--text-primary)]">
      <div className="border-2 border-[var(--border-primary)] bg-[var(--bg-secondary)] shadow-[4px_4px_0px_0px_var(--border-primary)] p-4">
        <h1 className="text-lg sm:text-xl font-black uppercase tracking-tight flex items-center gap-2">
          <Mail className="h-5 w-5 text-[var(--accent)]" /> Change Requests
        </h1>
      ...
      <div className="space-y-3">
        {requests.map((req) => (
          <div key={req._id} className="border-2 border-[var(--border-primary)] bg-[var(--bg-secondary)] shadow-[3px_3px_0px_0px_var(--border-primary)] p-4">
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0 space-y-1 flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-xs font-black uppercase">{req.userName}</span>
                  <span className={`text-[10px] font-bold px-1.5 py-0.5 border border-[var(--border-primary)] ${
                ...
                <div className="mt-2 grid grid-cols-2 gap-2 text-[10px]">
                  <div className="border border-[var(--border-primary)]/20 p-2 bg-white/50">
                ...
                {req.adminNote && (
                  <div className="mt-1 border border-[var(--border-primary)]/20 p-2 bg-white/50">
              ...
              {req.status === "pending" && (
                <div className="flex gap-2 shrink-0">
                  <button
                    ...
                    className="inline-flex h-8 items-center gap-1 rounded-none border-2 border-[var(--border-primary)] bg-green-200 px-3 text-[10px] font-black uppercase shadow-[2px_2px_0px_0px_var(--border-primary)] hover:bg-green-300 active:translate-x-[1px] active:translate-y-[1px] active:shadow-none"
                  >
                    <CheckCircle className="h-3 w-3" /> Approve
                  </button>
                  <button
                    onClick={() => { setActionId(req._id); setActionType("reject"); setActionNote(""); }}
                    className="inline-flex h-8 items-center gap-1 rounded-none border-2 border-[var(--border-primary)] bg-red-200 px-3 text-[10px] font-black uppercase shadow-[2px_2px_0px_0px_var(--border-primary)] hover:bg-red-300 active:translate-x-[1px] active:translate-y-[1px] active:shadow-none"
                  >
            ...
            {actionId === req._id && (
              <div className="mt-3 border-t-2 border-[var(--border-primary)] pt-3">
                <div className="flex items-end gap-2">
                  <div className="flex-1">
                    ...
                    <input
                      ...
                      className="w-full border-2 border-[var(--border-primary)] bg-[var(--surface-white)] px-2 py-1.5 text-xs font-bold outline-none"
                    />
                  </div>
                  <button
                    onClick={() => handleAction(req._id)}
                    className="inline-flex h-8 items-center gap-1 rounded-none border-2 border-[var(--border-primary)] bg-[var(--dark-bg)] px-3 text-[10px] font-black uppercase text-[var(--light-text)] shadow-[2px_2px_0px_0px_var(--border-primary)] hover:opacity-90 active:translate-x-[1px] active:translate-y-[1px] active:shadow-none"
                  >
                    <Clock className="h-3 w-3" /> Confirm
                  </button>
                  <button
                    onClick={() => { setActionId(null); setActionNote(""); setActionType(null); }}
                    className="inline-flex h-8 items-center rounded-none border-2 border-[var(--border-primary)] bg-[var(--surface-white)] px-3 text-[10px] font-black uppercase shadow-[2px_2px_0px_0px_var(--border-primary)] hover:bg-gray-100 active:translate-x-[1px] active:translate-y-[1px] active:shadow-none"
                  >
                    Cancel
                  </button>
                </div>
