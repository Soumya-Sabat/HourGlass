...
export default function CampusEstatePage() {
  ...
  return (
    <div className="space-y-6 font-mono text-[var(--text-primary)]">
      <div className="border-2 border-[var(--border-primary)] bg-[var(--bg-secondary)] shadow-[4px_4px_0px_0px_var(--border-primary)] p-4 flex items-center justify-between">
        <div>
          <h1 className="text-lg sm:text-xl font-black uppercase tracking-tight flex items-center gap-2">
            <Package2 className="h-5 w-5 text-[var(--accent)]" /> Campus Estate
          </h1>
      ...
      {showForm && (
        <div className="border-2 border-[var(--border-primary)] bg-[var(--bg-secondary)] shadow-[4px_4px_0px_0px_var(--border-primary)] p-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <table className="w-full border-collapse text-xs font-bold">
          ...
          <tbody>
            {classrooms.length === 0 ? (
              ...
              <tr key={c.id} className="border-b border-[var(--border-primary)]">
                <td className="p-3 font-black">{c.name}</td>
                <td className="p-3">{c.capacity}</td>
                <td className="p-3 text-[10px]">{c.resources}</td>
                <td className="p-3">{c.location}</td>
                <td className="p-3">
                  <button onClick={() => handleDelete(c.id)} className="p-1.5 border border-[var(--border-primary)] bg-red-200 hover:bg-red-300">
