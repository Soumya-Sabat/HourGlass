    ...
    <div className="space-y-6 font-mono text-[var(--text-primary)]">
      <div className="border-2 border-[var(--border-primary)] bg-[var(--bg-secondary)] shadow-[4px_4px_0px_0px_var(--border-primary)] p-4 flex items-center justify-between">
        <div>
          <h1 className="text-lg sm:text-xl font-black uppercase tracking-tight flex items-center gap-2">
            <Megaphone className="h-5 w-5 text-[var(--accent)]" /> Announcements
          </h1>
      ...
      {showForm && (
        <div className="border-2 border-[var(--border-primary)] bg-[var(--bg-secondary)] shadow-[4px_4px_0px_0px_var(--border-primary)] p-4 space-y-3">
          ...
          <textarea placeholder="Announcement Content" value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })}
            rows={4} className="w-full border-2 border-[var(--border-primary)] bg-[var(--bg-primary)] p-2 text-xs font-bold resize-none" />
      ...
      <div className="space-y-3">
        {announcements.length === 0 ? (
          <div className="border-2 border-[var(--border-primary)] bg-[var(--bg-secondary)] shadow-[4px_4px_0px_0px_var(--border-primary)] p-6 text-center text-xs font-black">No announcements yet.</div>
        ) : announcements.map((a) => (
          <div key={a.id} className="border-2 border-[var(--border-primary)] bg-[var(--bg-secondary)] shadow-[3px_3px_0px_0px_var(--border-primary)] p-4">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-2">
                  ...
                  <span className="px-2 py-0.5 bg-[var(--dark-bg)] text-[var(--light-text)] text-[9px] font-black uppercase">{a.status}</span>
                </div>
              </div>
              <button onClick={() => handleDelete(a.id)} className="p-1.5 border border-[var(--border-primary)] bg-red-200 hover:bg-red-300 shrink-0">
