...
export default function SubjectsPage() {
  ...
  if (loading) return <div className="border-2 border-[var(--border-primary)] bg-[var(--bg-secondary)] p-6 font-mono shadow-[4px_4px_0px_0px_var(--border-primary)]"><Loader className="h-4 w-4 animate-spin inline" /> Loading subjects...</div>;
  ...
  return (
    <div className="space-y-6 font-mono text-[var(--text-primary)]">
      <div className="border-2 border-[var(--border-primary)] bg-[var(--bg-secondary)] shadow-[4px_4px_0px_0px_var(--border-primary)] p-4 flex items-center justify-between">
        <div>
          <h1 className="text-lg sm:text-xl font-black uppercase tracking-tight flex items-center gap-2">
            <BookOpen className="h-5 w-5 text-[var(--accent)]" /> Subjects
          </h1>
      ...
      {showForm && (
        <div className="border-2 border-[var(--border-primary)] bg-[var(--bg-secondary)] shadow-[4px_4px_0px_0px_var(--border-primary)] p-4 space-y-3">
          <form onSubmit={handleAdd} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            <div>
              ...
              <input value={name} onChange={(e) => setName(e.target.value)} required
                className="w-full border-2 border-[var(--border-primary)] bg-[var(--bg-primary)] p-2 text-xs font-bold" placeholder="Data Structures" />
            </div>
            <div>
              <label className="text-[10px] font-black uppercase mb-1 block">Subject Code *</label>
              <input value={code} onChange={(e) => setCode(e.target.value)} required
                className="w-full border-2 border-[var(--border-primary)] bg-[var(--bg-primary)] p-2 text-xs font-bold uppercase" placeholder="CS201" />
            </div>
            <div>
              ...
              <input type="number" value={credits} onChange={(e) => setCredits(e.target.value)} min={0} max={20}
                className="w-full border-2 border-[var(--border-primary)] bg-[var(--bg-primary)] p-2 text-xs font-bold" placeholder="4" />
            </div>
            <div>
              ...
              <input type="number" value={semester} onChange={(e) => setSemester(e.target.value)} min={1} max={12}
                className="w-full border-2 border-[var(--border-primary)] bg-[var(--bg-primary)] p-2 text-xs font-bold" placeholder="3" />
            </div>
            <div>
            ...
            <div className="flex items-end gap-2">
              <button type="submit" disabled={saving}
                className="border-2 border-[var(--border-primary)] bg-[var(--dark-bg)] text-[var(--light-text)] px-4 py-2 text-xs font-black uppercase hover:bg-[var(--bg-primary)] disabled:opacity-50">
                {saving ? "Saving..." : "Save"}
              </button>
              <button type="button" onClick={() => setShowForm(false)}
                className="border-2 border-[var(--border-primary)] bg-[var(--surface-white)] px-4 py-2 text-xs font-black uppercase hover:bg-[var(--bg-secondary)]">
                Cancel
              </button>
            </div>
          </form>
