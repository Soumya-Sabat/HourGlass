...
export default function UsersPage() {
  ...
  if (loading) return <div className="border-2 border-[var(--border-primary)] bg-[var(--bg-secondary)] p-6 font-mono shadow-[4px_4px_0px_0px_var(--border-primary)]">Loading users...</div>;
    ...
    <div className="space-y-6 font-mono text-[var(--text-primary)]">
      <div className="border-2 border-[var(--border-primary)] bg-[var(--bg-secondary)] shadow-[4px_4px_0px_0px_var(--border-primary)] p-4">
        <h1 className="text-lg sm:text-xl font-black uppercase tracking-tight flex items-center gap-2">
          <Users className="h-5 w-5 text-[var(--accent)]" /> All Users
        </h1>
                    ...
                    {u.role !== "institution_admin" && (
                      <>
                        <button onClick={() => openEditModal(u)}
                          className="p-1.5 border border-[var(--border-primary)] bg-blue-200 hover:bg-blue-300"
                          title="Edit user">
                          <Pencil className="h-3 w-3" />
                        </button>
                        <button onClick={() => handleToggleStatus(u.id, u.status)}
                          className={`p-1.5 border border-[var(--border-primary)] ${u.status === "Active" ? "bg-red-200 hover:bg-red-300" : "bg-green-200 hover:bg-green-300"}`}
                        ...
                        <button onClick={() => { setRoleChangeId(u.id); setRoleChangeValue(u.role); }}
                          className="p-1.5 border border-[var(--border-primary)] bg-amber-200 hover:bg-amber-300"
                          title="Change role">
                          <Shield className="h-3 w-3" />
                        </button>
                        <button onClick={() => setDeleteId(u.id)}
                          className="p-1.5 border border-[var(--border-primary)] bg-red-200 hover:bg-red-300"
                          title="Delete user">
      ...
      {roleChangeId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setRoleChangeId(null)}>
          <div className="w-full max-w-sm border-2 border-[var(--border-primary)] bg-[var(--bg-primary)] p-6 shadow-[6px_6px_0px_0px_var(--border-primary)]" onClick={(e) => e.stopPropagation()}>
            ...
            <select value={roleChangeValue} onChange={(e) => setRoleChangeValue(e.target.value)}
              className="w-full border-2 border-[var(--border-primary)] bg-[var(--surface-white)] p-2 text-xs font-bold mb-4">
              {roleOptions.map((r) => (
            ...
            <div className="flex gap-2 justify-end">
              <button onClick={() => { setRoleChangeId(null); setRoleChangeError(""); }} className="border-2 border-[var(--border-primary)] bg-[var(--surface-white)] px-4 py-1.5 text-xs font-black hover:bg-[var(--bg-secondary)]">Cancel</button>
              <button onClick={() => roleChangeId && handleRoleChange(roleChangeId, roleChangeValue)} className="border-2 border-[var(--border-primary)] bg-[var(--accent)] px-4 py-1.5 text-xs font-black text-white hover:bg-[var(--accent)]">Save</button>
            </div>
          </div>
        </div>
      )}
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => !deleting && setDeleteId(null)}>
          <div className="w-full max-w-sm border-2 border-[var(--border-primary)] bg-[var(--bg-primary)] p-6 shadow-[6px_6px_0px_0px_var(--border-primary)]" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-2 justify-end">
              <button onClick={() => setDeleteId(null)} disabled={deleting} className="border-2 border-[var(--border-primary)] bg-[var(--surface-white)] px-4 py-1.5 text-xs font-black hover:bg-[var(--bg-secondary)] disabled:opacity-40">Cancel</button>
              <button onClick={handleDelete} disabled={deleting} className="border-2 border-[var(--border-primary)] bg-red-600 px-4 py-1.5 text-xs font-black text-white hover:bg-red-700 disabled:opacity-40">{deleting ? "Deleting..." : "Delete"}</button>
            </div>
          </div>
        </div>
      )}
            ...
            <form onSubmit={handleAddUser} className="space-y-3">
              ...
              <div>
                ...
                <input type="text" value={addClassGroup} onChange={(e) => setAddClassGroup(e.target.value)}
                  className="w-full border-2 border-[var(--border-primary)] bg-[var(--surface-white)] p-2 text-xs font-bold" placeholder="B.Tech, MBA, or leave blank" />
              </div>
                ...
                {!showNewDept ? (
                  <div className="flex gap-2">
                    <select value={addDepartment} onChange={(e) => setAddDepartment(e.target.value)}
                      className="flex-1 border-2 border-[var(--border-primary)] bg-[var(--surface-white)] p-2 text-xs font-bold">
                      <option value="">Select department...</option>
                      {departments.map((d) => <option key={d.id} value={d.name}>{d.name}</option>)}
                    </select>
                    <button type="button" onClick={() => { setShowNewDept(true); setAddDepartment("__new__"); }}
                      className="border-2 border-[var(--border-primary)] bg-[var(--bg-secondary)] px-3 text-xs font-black hover:bg-[var(--bg-secondary)] whitespace-nowrap">+ New</button>
                  </div>
                ) : (
                  <div className="space-y-2">
                    <input type="text" value={newDeptName} onChange={(e) => setNewDeptName(e.target.value)}
                      className="w-full border-2 border-[var(--border-primary)] bg-[var(--surface-white)] p-2 text-xs font-bold" placeholder="New department name" />
                    <button type="button" onClick={() => { setShowNewDept(false); setAddDepartment(""); }}
                      className="text-[10px] font-black text-[var(--accent)] hover:underline">Cancel</button>
                  </div>
                )}
              </div>
              {addError && <p className="text-xs font-bold text-red-600">{addError}</p>}
              <div className="flex gap-2 justify-end pt-2">
                <button type="button" onClick={() => setShowAdd(false)} disabled={adding} className="border-2 border-[var(--border-primary)] bg-[var(--surface-white)] px-4 py-1.5 text-xs font-black hover:bg-[var(--bg-secondary)] disabled:opacity-40">Cancel</button>
                <button type="submit" disabled={adding} className="border-2 border-[var(--border-primary)] bg-[var(--accent)] px-4 py-1.5 text-xs font-black text-white hover:bg-[var(--accent)] disabled:opacity-40">{adding ? "Adding..." : "Add User"}</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => !editing && setEditUser(null)}>
          <div className="w-full max-w-md border-2 border-[var(--border-primary)] bg-[var(--bg-primary)] p-6 shadow-[6px_6px_0px_0px_var(--border-primary)]" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleEditUser} className="space-y-3">
              ...
              <div className="flex gap-2 justify-end pt-2">
                <button type="button" onClick={() => setEditUser(null)} disabled={editing} className="border-2 border-[var(--border-primary)] bg-[var(--surface-white)] px-4 py-1.5 text-xs font-black hover:bg-[var(--bg-secondary)] disabled:opacity-40">Cancel</button>
