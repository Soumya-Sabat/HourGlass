...
export default function ActivityLogPage() {
  ...
  if (loading) return <div className="border-2 border-black bg-[#eae3cb] p-6 font-mono shadow-[4px_4px_0px_0px_#1a1a14]">Loading activity log...</div>;

  return (
    <div className="space-y-6 font-mono text-[#1a1a14]">
      <div className="border-2 border-black bg-[#eae3cb] shadow-[4px_4px_0px_0px_#1a1a14] p-4">
        <h1 className="text-lg sm:text-xl font-black uppercase tracking-tight flex items-center gap-2">
          <Activity className="h-5 w-5 text-[#e28774]" /> Activity Log
        </h1>
          ...
          <tbody>
            {filtered.length === 0 ? (
              ...
              <tr key={l.id} className="border-b border-black">
                ...
                <td className="p-3">
                  <span className="px-2 py-0.5 bg-[#1a1a14] text-[#f4ebd0] text-[10px] font-black uppercase">{l.action}</span>
                </td>
