...
export default function DepartmentsPage() {
  ...
  if (loading) return <div className="border-2 border-[var(--border-primary)] bg-[var(--bg-secondary)] p-6 font-mono shadow-[4px_4px_0px_0px_var(--border-primary)]"><Loader className="h-4 w-4 animate-spi" /> Loading...</div>;
      ...
      <div className="border-2 border-[var(--border-primary)] bg-[var(--bg-secondary)] shadow-[4px_4px_0px_0px_var(--border-primary)] p-4 flex items-center justify-between">
        <div>
          <h1 className="text-lg sm:text-xl font-black uppercase tracking-tight flex items-center gap-2">
            <Layers className="h-5 w-5 text-[var(--accent)]" /> Departments
          </h1>
      ...
      {showForm && (
        <div className="border-2 border-[var(--border-primary)] bg-[var(--bg-secondary)] shadow-[4px_4px_0px_0px_var(--border-primary)] p-4 space-y-3">
          ...
          <button onClick={handleAdd}
            className="border-2 border-[var(--border-primary)] bg-[var(--dark-bg)] text-[var(--light-text)] px-4 py-2 text-xs font-black uppercase">
            Save Department
          </button>
        </div>
      )}
        ...
        <table className="w-full border-collapse text-xs font-bold">
          ...
          <tbody>
            {departments.length === 0 ? (
              ...
              <tr key={d.id} className="border-b border-[var(--border-primary)]">
