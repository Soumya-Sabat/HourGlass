...
export default function ClusterChatPage() {
  ...
  return (
    <div className="space-y-4 font-mono text-[#1a1a14]">
      <div className="border-2 border-black bg-[#eae3cb] shadow-[4px_4px_0px_0px_#1a1a14] p-4 flex items-center gap-3">
        <Link href="/dashboard/faculty/clusters"
          className="border border-black p-1.5 bg-white hover:bg-[#e28774] transition-colors">
          <ArrowLeft className="h-4 w-4" />
        </Link>
        <div>
          <h1 className="text-sm font-black uppercase tracking-tight flex items-center gap-2">
            <MessageSquare className="h-4 w-4 text-[#e28774]" /> Cluster Chat
          </h1>
      ...
      <div className="border-2 border-black bg-[#eae3cb] shadow-[4px_4px_0px_0px_#1a1a14] flex flex-col">
        <div className="p-4 bg-white space-y-3 max-h-[500px] overflow-y-auto min-h-[300px]">
          {loading ? (
            ...
            messages.map((msg) => (
              <div key={msg.id} className="border border-black p-2.5 bg-[#f4ebd0]/30">
                <div className="flex justify-between items-start gap-2 mb-1">
                  <span className="text-[10px] font-black uppercase text-[#e28774]">{msg.senderName}</span>
        ...
        <div className="border-t-2 border-black p-3 bg-[#eae3cb] flex gap-2">
          <textarea
            value={input}
            ...
            rows={1}
            className="flex-1 border-2 border-black bg-[#f4ebd0] p-2 text-xs font-bold resize-none focus:outline-none"
          />
          <button onClick={handleSend} disabled={sending || !input.trim()}
            className="border-2 border-black bg-[#e28774] px-4 py-2 text-xs font-black uppercase hover:bg-[#d17664] transition-colors disabled:opacity-50 flex items-center gap-1">
