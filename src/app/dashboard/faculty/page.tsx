    ...
    <div className="space-y-6 font-mono text-[#1a1a14]">
      <div className="border-2 border-black bg-[#eae3cb] shadow-[4px_4px_0px_0px_#1a1a14] p-4">
        <h1 className="text-lg sm:text-xl font-black uppercase tracking-tight flex items-center gap-2">
          <UserSquare2 className="h-5 w-5 text-[#e28774]" /> Faculty Core
        </h1>
      ...
      <div className="border-2 border-black bg-[#eae3cb] shadow-[3px_3px_0px_0px_#1a1a14] p-4">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-black uppercase flex items-center gap-2">
            <Pencil className="h-4 w-4 text-[#e28774]" /> My Profile
          </h2>
        ...
        {editing ? (
          <div className="space-y-2">
            <input value={editName} onChange={(e) => setEditName(e.target.value)}
              className="w-full border-2 border-black bg-[#f4ebd0] p-2 text-xs font-bold" placeholder="Full name" />
            <input value={editPhone} onChange={(e) => setEditPhone(e.target.value)}
              className="w-full border-2 border-black bg-[#f4ebd0] p-2 text-xs font-bold" placeholder="Phone number" />
            <input value={editDesignation} onChange={(e) => setEditDesignation(e.target.value)}
              className="w-full border-2 border-black bg-[#f4ebd0] p-2 text-xs font-bold" placeholder="Designation" />
            <input value={editExperience} onChange={(e) => setEditExperience(e.target.value)}
              className="w-full border-2 border-black bg-[#f4ebd0] p-2 text-xs font-bold" placeholder="Years of Experience" />
            <button onClick={handleSaveProfile} disabled={saveLoading}
              className="border-2 border-black bg-[#e28774] text-xs font-black uppercase px-4 py-2 shadow-[2px_2px_0px_0px_#1a1a14] hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] transition-all disabled:opacity-50">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-[10px] font-bold">
            <div className="border border-black bg-white p-2"><span className="text-gray-500">Name</span><br />{profile?.fullName || "—"}</div>
            <div className="border border-black bg-white p-2"><span className="text-gray-500">Email</span><br />{profile?.email || "—"}</div>
            <div className="border border-black bg-white p-2"><span className="text-gray-500">Phone</span><br />{profile?.phoneNumber || "—"}</div>
            <div className="border border-black bg-white p-2"><span className="text-gray-500">Designation</span><br />{profile?.designation || "—"}</div>
            <div className="border border-black bg-white p-2"><span className="text-gray-500">Dept</span><br />{profile?.department || "—"}</div>
            <div className="border border-black bg-white p-2"><span className="text-gray-500">Experience</span><br />{profile?.yearsOfExperience || "—"}</div>
          </div>
        )}
      </div>
      ...
      {clusters.length > 0 && (
        <div className="border-2 border-black bg-[#eae3cb] shadow-[3px_3px_0px_0px_#1a1a14] p-4">
          <h2 className="text-sm font-black uppercase flex items-center gap-2 mb-3">
            <Users className="h-4 w-4 text-[#e28774]" /> My Clusters
          </h2>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="border-2 border-black bg-[#eae3cb] shadow-[3px_3px_0px_0px_#1a1a14] p-4">
          <h2 className="text-sm font-black uppercase flex items-center gap-2 mb-3">
            <GraduationCap className="h-4 w-4 text-[#e28774]" /> Upcoming Classes
          </h2>
          {(stats?.upcomingClasses ?? []).length === 0 ? (
            ...
            <div className="space-y-2">
              {stats?.upcomingClasses.map((c, i) => (
                <div key={i} className="border border-black bg-white p-2 text-[10px] font-bold">
          ...
          <Link href="/dashboard/faculty/timetable"
            className="inline-block mt-2 text-[9px] font-black uppercase border border-black px-2 py-1 bg-white hover:bg-[#e28774] transition-colors">
          ...
          {(stats?.recentNotices ?? []).length === 0 ? (
            ...
            <div className="space-y-2">
              {stats?.recentNotices.map((n) => (
                <div key={n.id} className="border border-black bg-white p-2 text-[10px] font-bold">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Link href="/dashboard/faculty/attendance"
          className="border-2 border-black bg-[#e28774] shadow-[3px_3px_0px_0px_#1a1a14] p-3 text-center text-xs font-black uppercase hover:shadow-none hover:translate-x-[3px] hover:translate-y-[3px] transition-all">
          <ClipboardCheck className="h-4 w-4 mx-auto mb-1" /> Mark Attendance
        </Link>
        <Link href="/dashboard/faculty/downloads"
          className="border-2 border-black bg-[#f4ebd0] shadow-[3px_3px_0px_0px_#1a1a14] p-3 text-center text-xs font-black uppercase hover:shadow-none hover:translate-x-[3px] hover:translate-y-[3px] transition-all">
