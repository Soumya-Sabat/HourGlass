...
import { getHeadNotices, type HeadNotice } from "@/actions/head-actions";

export default function HeadNoticesPage() {
  const [notices, setNotices] = useState<HeadNotice[]>([]);
  const [loading, setLoading] = useState(true);
  const [expanded, setExpanded] = useState<string | null>(null);

  useEffect(() => {
    getHeadNotices().then(setNotices).catch(() => {}).finally(() => setLoading(false));
  }, []);
    ...
    <div className="space-y-6 font-mono text-[var(--text-primary)]">
      <div className="border-2 border-[var(--border-primary)] bg-[var(--bg-secondary)] shadow-[4px_4px_0px_0px_var(--border-primary)] p-4">
        <h1 className="text-lg sm:text-xl font-black uppercase tracking-tight flex items-center gap-2">
          <Bell className="h-5 w-5 text-[var(--accent)]" /> Department Notices
        </h1>
        <p className="text-[10px] font-bold text-gray-600 mt-1">Review &amp; publish official notices</p>
      </div>
