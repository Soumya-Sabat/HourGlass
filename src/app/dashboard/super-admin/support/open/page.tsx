...
export default function OpenTicketsPage() {
  ...
  async function fetchData() {
    try {
      const res = await fetch("/api/super-admin/support/tickets?status=Open");
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Open Tickets"
        description="Tickets awaiting initial response or assignment"
        actions={
      ...
      <Table
        ...
        rows={data.map((t: any) => [
          ...
          <div key="actions" className="flex items-center gap-1">
            <ActionButton label="Assign" variant="default" icon={UserCheck} />
