...
export default function SuspendedInstitutionsPage() {
  ...
  async function fetchData() {
    try {
      const res = await fetch("/api/super-admin/institutions?status=suspended");
  ...
  const rows = data.map((inst: any) => [
    ...
    <StatusBadge key="s" status="Suspended" />,
    <span key="d">{inst.createdAt ? new Date(inst.createdAt).toISOString().split("T")[0] : "—"}</span>,
    <div key="a" className="flex gap-1">
      <ActionButton label="Reactivate" variant="primary" icon={RefreshCw} />
    </div>,
  ]);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Institutions that have been suspended from the platform"
        actions={
