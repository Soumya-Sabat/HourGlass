...
export default function ExamsPage() {
  ...
  if (loading) return <div className="border-2 border-black bg-[#eae3cb] p-6 font-mono shadow-[4px_4px_0px_0px_#1a1a14]">Loading exams...</div>;
    ...
    <div className="space-y-6 font-mono text-[#1a1a14]">
      <div className="border-2 border-black bg-[#eae3cb] shadow-[4px_4px_0px_0px_#1a1a14] p-4">
        <h1 className="text-lg sm:text-xl font-black uppercase tracking-tight flex items-center gap-2">
          <ClipboardList className="h-5 w-5 text-[#e28774]" /> Exam Schedules
        </h1>
          ...
          <tbody>
            {exams.length === 0 ? (
              ...
              <tr key={e.id} className="border-b border-black">
                ...
                <td className="p-3">
                  <button onClick={() => handleDelete(e.id)} className="p-1.5 border border-black bg-red-200 hover:bg-red-300" title="Delete">
            ...
            <form onSubmit={handleAdd} className="space-y-3">
              <div>
                ...
                <input type="text" required value={newTitle} onChange={(e) => setNewTitle(e.target.value)}
                  className="w-full border-2 border-black bg-white p-2 text-xs font-bold" placeholder="Mid Term Exam" />
              </div>
              <div>
              ...
              <div className="flex gap-2 justify-end pt-2">
                ...
                <button type="submit" disabled={saving} className="border-2 border-black bg-[#e28774] px-4 py-1.5 text-xs font-black text-white hover:bg-[#d97766] disabled:opacity-40">{saving ? "Scheduling..." : "Schedule"}</button>
              </div>
            </form>
          </div>
        </div>
